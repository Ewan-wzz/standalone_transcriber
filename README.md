...
这是一个与 NoteAI 数据库和页面完全独立的本地工具。浏览器扩展只解析当前小红书视频地址，本地程序负责下载视频、提取音轨并使用 SenseVoice 离线转写。
...
4. 打开小红书视频笔记，点击扩展并选择“提取当前视频文案”。

本地服务只监听 `127.0.0.1:8765`。插件复用 NoteAI 的视频采集方式，在当前小红书页面上下文中读取视频，再把视频本体交给本机服务；本机服务不再解析、下载或校验视频 URL。转写结果保存到 `output` 文件夹。

完整压缩包解压后可直接双击 `XHSOfflineTranscriber.exe`。桌面程序会自动启动插件所需的本地服务，并提供本地视频转写、打开插件目录、打开结果目录和复制文本功能；不需要 Python，也不需要联网。首次安装开发版时运行 `setup.ps1` 需要联网下载并校验第三方运行文件。
